...
export default function ServerToggle() {
  ...
  useEffect(() => {
    ...
    const fetchServerStatus = async () => {
      try {
        ...
        showAlert(
          error.response?.data?.message || "Server status ဖတ်ရန်မအောင်မြင်ပါ",
          "error",
        );
      } finally {
        setLoading(false);
  ...
  const toggleServer = async (newStatus) => {
    try {
      ...
      setIsOpen(newStatus);
      showAlert(
        res.data?.message || `Server is now ${newStatus ? "Open" : "Closed"}!`,
        "success",
      );
    } catch (error) {
      console.error(error);
      showAlert(
        error.response?.data?.message || "Server update မအောင်မြင်ပါ ",
        "error",
      );
    }
  };
  ...
  const handlePasswordSubmit = async () => {
    ...
    try {
      ...
      console.error(error);
      showAlert(
        error.response?.data?.message || "Passcode စစ်ဆေးမှု မအောင်မြင်ပါ ",
        "error",
      );
    }
  };
