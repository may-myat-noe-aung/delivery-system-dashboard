...
export default function EditAccountTab() {
  ...
  const isValidPhone = (phone) => /^\d{7,15}$/.test(phone);
  const handleSaveClick = () => { if (!account) return; setShowModal(true); };
  const cancelModal = () => { setShowModal(false); setPassword(""); };

  const handlePasswordSubmit = async () => {
    if (!password) { showAlert("Passcode ထည့်ပေးပါ", "warning"); return; }
    if (!isValidPhone(account.phone)) { showAlert("Phone Number သည် မှန်ကန်သော format မဟုတ်ပါ", "warning"); return; }
    try {
      const verify = await axios.post(`${API_BASE}/admin/verify-owner-passcode`, { passcode: password });
      if (!verify.data.success) { showAlert(verify.data.message, "error"); return; }

      setSaving(true);
      const formData = new FormData();
      Object.entries({ strid: account.id, name: account.name, email: account.email, phone: account.phone, gender: account.gender, role: account.role })
        .forEach(([k, v]) => formData.append(k, v));
      ...
      const res = await axios.put(`${API_BASE}/admin`, formData);
      showAlert(res.data.message, res.data.success ? "success" : "error");
      if (res.data.success) cancelModal();
    } catch (err) {
      showAlert(err.response?.data?.message || "Error occurred", "error");
    } finally {
      setSaving(false);
  ...
  if (loading) return <p className={`text-gray-500 ${isDark ? "text-gray-400" : ""}`}>Loading accounts...</p>;

  return (
    <div className={`${isDark ? "text-gray-200" : "text-gray-800"}`}>
      <div className="flex justify-between mb-6">
        <h3 className={`font-bold text-xl ${isDark ? "text-purple-400" : "text-[#B476FF]"}`}>Edit Account</h3>

        {/* Email Dropdown */}
        <div className="w-[300px] relative" ref={dropdownRef}>
          <div
            className={`w-full rounded-lg border px-3 py-2 text-sm cursor-pointer
              ${isDark ? "bg-gray-800 border-gray-700 text-gray-200 hover:border-purple-400" : "bg-white border-gray-300 text-gray-700 hover:border-[#B476FF]"}`}
            onClick={() => setDropdownOpen(!dropdownOpen)}
          >
            {selectedEmail || "Select account email"}
          </div>
          ...
          {dropdownOpen && (
            <ul className={`absolute z-50 w-full rounded-lg mt-1 max-h-48 overflow-auto
              ${isDark ? "bg-gray-800 border-gray-700" : "bg-white border-gray-300"}`}>
              {accounts.map((a) => (
                <li
                  key={a.id}
                  onClick={() => { setSelectedEmail(a.email); setDropdownOpen(false); }}
                  className={`px-3 py-2 cursor-pointer ${isDark ? "hover:bg-purple-600 hover:text-white" : "hover:bg-[#B476FF] hover:text-white"}`}
                >
                  {a.email}
                </li>
              ))}
      ...
      {account && (
        <div className="grid grid-cols-2 gap-6">
          <div>
            <label className={`text-sm block mb-1 ${isDark ? "text-gray-300" : "text-gray-600"}`}>Name *</label>
            <input
              ref={nameRef}
              value={account.name || ""}
              onChange={(e) => setAccount({ ...account, name: e.target.value })}
              className={`w-full rounded-lg px-3 py-2 mt-1 border focus:outline-none focus:ring-2
                ${isDark ? "bg-gray-700 border-gray-600 text-white focus:ring-purple-400" : "bg-white border-gray-300 text-gray-700 focus:ring-[#B476FF]"}`}
            />
          </div>

          <div>
            <label className={`text-sm block mb-1 ${isDark ? "text-gray-300" : "text-gray-600"}`}>Phone *</label>
            <input
              ref={phoneRef}
              value={account.phone || ""}
          ...
          <div className="col-span-2">
            <label className={`text-sm block mb-2 ${isDark ? "text-gray-300" : "text-gray-600"}`}>Gender *</label>
            <div className="flex gap-4">
              {["male", "female", "other"].map((g) => (
                <label key={g} className="flex items-center gap-2">
                  <input
          ...
          <div className="col-span-2 flex justify-end gap-3 pt-4">
            <button
              onClick={() => setSelectedEmail("")}
              className={`px-5 py-2 rounded-lg text-sm border ${isDark ? "bg-gray-700 text-gray-200 hover:bg-gray-600" : "bg-gray-200 text-gray-700 hover:bg-gray-100"}`}
            >
              Cancel
            </button>
            <button
              onClick={handleSaveClick}
              disabled={saving}
              className={`px-5 py-2 rounded-lg text-sm text-white ${isDark ? "bg-purple-500 hover:bg-purple-600" : "bg-[#B476FF] hover:bg-[#B476FF]"}`}
            >
              {saving ? "Saving..." : "Save Changes"}
            </button>
          </div>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className={`relative w-80 p-6 rounded-xl border
            ${isDark ? "bg-gray-800 border-gray-700 text-gray-200" : "bg-white border-gray-300 text-gray-800"}`}>
            <button
              onClick={cancelModal}
              className={`absolute top-2 right-2 ${isDark ? "text-gray-400 hover:text-white" : "text-gray-400 hover:text-black"}`}
            >
              <X size={18} />
            </button>

            <h3 className={`text-lg font-semibold mb-4 text-center ${isDark ? "text-purple-400" : "text-[#B476FF]"}`}>
              Enter Owner Passcode
            </h3>
            ...
            <input
              ...
              autoFocus
              onKeyDown={(e) => e.key === "Enter" && handlePasswordSubmit()}
              className={`w-full px-3 py-2 mb-4 rounded-lg border focus:outline-none focus:ring-2
                ${isDark ? "bg-gray-700 border-gray-600 text-white focus:ring-purple-400" : "bg-white border-gray-300 text-gray-800 focus:ring-[#B476FF]"}`}
            />

            <div className="flex justify-between">
              <button
                onClick={cancelModal}
                className={`px-3 py-2 rounded-md text-sm ${isDark ? "bg-gray-700 text-gray-200 hover:bg-gray-600" : "bg-gray-200 text-gray-700 hover:bg-gray-100"}`}
              >
                Cancel
              </button>
              <button
                onClick={handlePasswordSubmit}
                className={`px-3 py-2 rounded-md text-sm text-white ${isDark ? "bg-purple-500 hover:bg-purple-600" : "bg-[#B476FF] hover:bg-[#B476FF]"}`}
              >
                Confirm
              </button>
            </div>
