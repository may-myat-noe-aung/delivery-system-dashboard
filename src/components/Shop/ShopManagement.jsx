...
const ShopManagement = ({ shops, onDetail, onViewShopDetails }) => {
  ...
  // SAFE FILTER
  // =========================
  // 🔍 SAFE FILTER (multi-field search)
  const filteredShops = shops.filter((shop) => {
    ...
    return (
      (shop.shop_name || "").toString().toLowerCase().includes(term) ||
      (shop.shopkeeper_name || "").toString().toLowerCase().includes(term) ||
      (shop.phone || "").toString().toLowerCase().includes(term) ||
      (shop.email || "").toString().toLowerCase().includes(term)
    );
  });
